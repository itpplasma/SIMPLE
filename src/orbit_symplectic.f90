module orbit_symplectic
    ...
    use orbit_symplectic_base, only: orbit_timestep_sympl_i, extrap_field
    use orbit_symplectic_base, only: coeff_rk_gauss, coeff_rk_lobatto, f_rk_lobatto
    use orbit_symplectic_quasi, only: orbit_timestep_quasi, &
                                      timestep_expl_impl_euler_quasi, &
                                      timestep_impl_expl_euler_quasi, &
                                      timestep_midpoint_quasi, &
                                      orbit_timestep_rk45, &
...
contains
    ...
    subroutine f_sympl_euler1(si, f, n, x, fvec, iflag)
        ...
        fvec(1) = f%dpth(1)*(f%pth - si%pthold) + si%dt*(f%dH(2)*f%dpth(1) - &
                                                         f%dH(1)*f%dpth(2))
        fvec(2) = f%dpth(1)*(x(2) - si%z(4)) + si%dt*(f%dH(3)*f%dpth(1) - &
                                                      f%dH(1)*f%dpth(3))
    ...
    subroutine f_sympl_euler2(si, f, n, x, fvec, iflag)
        ...
        fvec(3) = f%dpth(1)*f%hph*(x(3) - si%z(3)) - si%dt*(f%dpth(1)*f%vpar - &
                                                            f%dH(1)*f%hth)
    ...
    subroutine jac_sympl_euler2(si, f, x, jac)
        ...
        jac(3, :) = (f%d2pth(1:3)*f%hph + f%dpth(1)*f%dhph)*(x(3) - si%z(3)) &
                    - si%dt*(f%d2pth(1:3)*f%vpar + f%dpth(1)*f%dvpar(1:3) - &
                             f%d2H(1:3)*f%hth - f%dH(1)*f%dhth)
    ...
    subroutine f_midpoint_part1(si, f, n, x, fvec)
        ...
        fvec(3) = f%dpth(1)*f%hph*(x(3) - si%z(3)) - si%dt*(f%dpth(1)*f%vpar - &
                                                            f%dH(1)*f%hth)
        fvec(4) = f%dpth(1)*(x(4) - si%z(4)) + si%dt*(f%dH(3)*f%dpth(1) - &
                                                      f%dH(1)*f%dpth(3))
        fvec(5) = f%dpth(1)*(f%pth - si%pthold) + 0.5d0*si%dt*(f%dpth(1)*f%dH(2) - &
                                                               f%dpth(2)*f%dH(1))
    ...
    subroutine jac_midpoint_part1(si, f, x, jac)
        ...
        jac(3, 2:3) = 0.5d0*((f%d2pth(2:3)*f%hph + f%dpth(1)*f%dhph(2:3))*(x(3) &
                                                                           - si%z(3)) &
    ...
    subroutine jac_midpoint_part2(si, f, fmid, x, jac)
        ...
        jac(1, 2) = 0.5d0*(fmid%d2pth(2)*(f%pth - si%pthold) &
                           + si%dt*(fmid%d2pth(2)*fmid%dH(2) + &
                                    fmid%dpth(1)*fmid%d2H(4) &
                                    - fmid%dpth(2)*fmid%d2H(2) - &
                                    fmid%d2pth(4)*fmid%dH(1))) + &
                    fmid%dpth(1)*f%dpth(2)
        jac(1, 3) = 0.5d0*(fmid%d2pth(3)*(f%pth - si%pthold) &
                           + si%dt*(fmid%d2pth(3)*fmid%dH(2) + &
                                    fmid%dpth(1)*fmid%d2H(5) &
                                    - fmid%dpth(2)*fmid%d2H(3) - &
                                    fmid%d2pth(5)*fmid%dH(1))) + &
                    fmid%dpth(1)*f%dpth(3)
        jac(1, 4) = 0.5d0*(fmid%d2pth(7)*(f%pth - si%pthold) &
                           + si%dt*(fmid%d2pth(7)*fmid%dH(2) + &
                                    fmid%dpth(1)*fmid%d2H(8) &
                                    - fmid%dpth(2)*fmid%d2H(7) - &
                                    fmid%d2pth(8)*fmid%dH(1))) + &
    ...
    subroutine newton2(si, f, x, atol, rtol, maxit, xlast)
        ...
        do kit = 1, maxit
            ...
            det = fjac(1, 1)*fjac(2, 2)*fjac(3, 3) + fjac(1, 2)*fjac(2, &
                                                                     3)*fjac(3, 1) + &
                  fjac(1, 3)*fjac(2, 1)*fjac(3, 2) &
                  - fjac(1, 3)*fjac(2, 2)*fjac(3, 1) - fjac(1, 1)*fjac(2, &
                                                                       3)*fjac(3, 2) - &
                  fjac(1, 2)*fjac(2, 1)*fjac(3, 3)
        ...
        print *, 'newton2: maximum iterations reached: ', maxit, 'z = ', x(1), x(2), &
            x(3), si%z(4)
    ...
    subroutine f_rk_gauss(si, fs, s, x, fvec)
        ...
        do k = 1, s
            ...
            do l = 1, s
                fvec(4*k - 3) = fvec(4*k - 3) + si%dt*a(k, l)*(fs(l)%dH(2) - &
                                                               Hprime(l)*fs(l)%dpth(2))
                ! pthdot
                fvec(4*k - 2) = fvec(4*k - 2) - si%dt*a(k, l)*Hprime(l)
                ! thdot
                fvec(4*k - 1) = fvec(4*k - 1) - si%dt*a(k, l)* &
                                (fs(l)%vpar - Hprime(l)*fs(l)%hth)/fs(l)%hph
                ! phdot
                fvec(4*k) = fvec(4*k) + si%dt*a(k, l)*(fs(l)%dH(3) - &
        ...
        do l = 1, s
            do k = 1, s
                ...
                jac(m - 3, 4*l - 3) = jac(m - 3, 4*l - 3) & ! d/dr
                                      + si%dt*a(k, l)*(fs(l)%d2H(2) - &
                                                       fs(l)%d2pth(2)*Hprime(l) - &
                                                       fs(l)%dpth(2)*dHprime(4*l - 3))
                jac(m - 3, 4*l - 2) = jac(m - 3, 4*l - 2) & ! d/dth
                                      + si%dt*a(k, l)*(fs(l)%d2H(4) - &
                                                       fs(l)%d2pth(4)*Hprime(l) - &
                                                       fs(l)%dpth(2)*dHprime(4*l - 2))
                jac(m - 3, 4*l - 1) = jac(m - 3, 4*l - 1) & ! d/dph
                                      + si%dt*a(k, l)*(fs(l)%d2H(5) - &
                                                       fs(l)%d2pth(5)*Hprime(l) - &
                                                       fs(l)%dpth(2)*dHprime(4*l - 1))
                jac(m - 3, 4*l) = jac(m - 3, 4*l) & ! d/dpph
                                  + si%dt*a(k, l)*(fs(l)%d2H(8) - &
                                                   fs(l)%d2pth(8)*Hprime(l) &
                                                   - fs(l)%dpth(2)*dHprime(4*l))

                jac(m - 2, 4*l - 3) = jac(m - 2, 4*l - 3) - si%dt*a(k, &
                                                                    l)*dHprime(4*l - 3)
                ! d/dr
                jac(m - 2, 4*l - 2) = jac(m - 2, 4*l - 2) - si%dt*a(k, &
                                                                    l)*dHprime(4*l - 2)
                ! d/dth
                jac(m - 2, 4*l - 1) = jac(m - 2, 4*l - 1) - si%dt*a(k, &
                                                                    l)*dHprime(4*l - 1)
                ! d/dpph
                ...
                ! d/dpph
                jac(m - 1, 4*l) = jac(m - 1, 4*l) - si%dt*a(k, l)* &
                                  (fs(l)%dvpar(4) - dHprime(4*l)*fs(l)%hth)/fs(l)%hph

                jac(m, 4*l - 3) = jac(m, 4*l - 3) & ! d/dr
                                  + si%dt*a(k, l)*(fs(l)%d2H(3) - &
                                                   fs(l)%d2pth(3)*Hprime(l) &
                                                   - fs(l)%dpth(3)*dHprime(4*l - 3))
                jac(m, 4*l - 2) = jac(m, 4*l - 2) & ! d/dth
                                  + si%dt*a(k, l)*(fs(l)%d2H(5) - &
                                                   fs(l)%d2pth(5)*Hprime(l) &
                                                   - fs(l)%dpth(3)*dHprime(4*l - 2))
                jac(m, 4*l - 1) = jac(m, 4*l - 1) & ! d/dph
                                  + si%dt*a(k, l)*(fs(l)%d2H(6) - &
                                                   fs(l)%d2pth(6)*Hprime(l) &
                                                   - fs(l)%dpth(3)*dHprime(4*l - 1))
                jac(m, 4*l) = jac(m, 4*l) & ! d/dpph
                              + si%dt*a(k, l)*(fs(l)%d2H(9) - &
                                               fs(l)%d2pth(9)*Hprime(l) - &
                                               fs(l)%dpth(3)*dHprime(4*l))
            end do
        end do
    ...
    subroutine newton_rk_gauss(si, fs, s, x, atol, rtol, maxit, xlast)
        ...
        print *, 'newton_rk_gauss: maximum iterations reached: ', maxit, 'z = ', &
            x(1), x(2), x(3), si%z(4)
    ...
    subroutine fixpoint_rk_gauss(si, fs, s, x, atol, rtol, maxit, xlast)
        ...
        do kit = 1, 4096
            ...
            dpthnewdr = -si%dt*a(k, l)*( &
                        fs(l)%d2H(2) - Hprime(l)*fs(l)%d2pth(2) - &
                        dHprimedr(l)*fs(l)%dpth(2))
            damp = 0.99d0

            x(1) = x(1) - (1d0 - damp)*pthnew/dpthnewdr
            x(2) = damp*x(2) + (1d0 - damp)*(si%z(2) + &
                                             si%dt*a(k, l)*si%dt*a(k, l)*Hprime(l))
            x(3) = damp*x(3) + (1d0 - damp)*(si%z(3) + si%dt*a(k, l)* &
                                             (fs(l)%vpar - Hprime(l)*fs(l)%hth)/ &
                                             fs(l)%hph)
            x(4) = damp*x(4) + (1d0 - damp)*(si%z(4) - si%dt*a(k, l)* &
        ...
        print *, 'fixpoint_rk_gauss: maximum iterations reached: ', maxit, &
            'z = ', x(1), &
            x(2), x(3), si%z(4)
        write (6603, *) x, xabs, fvec
    ...
    subroutine jac_rk_lobatto(si, fs, s, jac)
        ...
        jac(1, 1) = jac(1, 1) + si%dt*ahat(1, 1)* &
                    (fs(1)%d2H(2) - fs(1)%d2pth(2)*Hprime(1) - &
                     fs(1)%dpth(2)*dHprime(1))
        jac(1, 2) = jac(1, 2) + si%dt*ahat(1, 1)* &
                    (fs(1)%d2H(8) - fs(1)%d2pth(8)*Hprime(1) - &
                     fs(1)%dpth(2)*dHprime(2))
        jac(2, 1) = jac(2, 1) + si%dt*ahat(1, 1)* &
                    (fs(1)%d2H(3) - fs(1)%d2pth(3)*Hprime(1) - &
                     fs(1)%dpth(3)*dHprime(1))
        jac(2, 2) = jac(2, 2) + si%dt*ahat(1, 1)* &
                    (fs(1)%d2H(9) - fs(1)%d2pth(9)*Hprime(1) - &
                     fs(1)%dpth(3)*dHprime(2))

        do l = 2, s
            n = 4*l - 2
            jac(1, n - 3) = jac(1, n - 3) + si%dt*ahat(1, l)* &
                            (fs(l)%d2H(2) - fs(l)%d2pth(2)*Hprime(l) - &
                             fs(l)%dpth(2)*dHprime(n - 3))
            jac(1, n - 2) = jac(1, n - 2) + si%dt*ahat(1, l)* &
                            (fs(l)%d2H(4) - fs(l)%d2pth(4)*Hprime(l) - &
                             fs(l)%dpth(2)*dHprime(n - 2))
            jac(1, n - 1) = jac(1, n - 1) + si%dt*ahat(1, l)* &
                            (fs(l)%d2H(5) - fs(l)%d2pth(5)*Hprime(l) - &
                             fs(l)%dpth(2)*dHprime(n - 1))
            jac(1, n) = jac(1, n) + si%dt*ahat(1, l)* &
                        (fs(l)%d2H(8) - fs(l)%d2pth(8)*Hprime(l) - &
                         fs(l)%dpth(2)*dHprime(n))

            jac(2, n - 3) = jac(2, n - 3) + si%dt*ahat(1, l)* &
                            (fs(l)%d2H(3) - fs(l)%d2pth(3)*Hprime(l) - &
                             fs(l)%dpth(3)*dHprime(n - 3))
            jac(2, n - 2) = jac(2, n - 2) + si%dt*ahat(1, l)* &
                            (fs(l)%d2H(5) - fs(l)%d2pth(5)*Hprime(l) - &
                             fs(l)%dpth(3)*dHprime(n - 2))
            jac(2, n - 1) = jac(2, n - 1) + si%dt*ahat(1, l)* &
                            (fs(l)%d2H(6) - fs(l)%d2pth(6)*Hprime(l) - &
                             fs(l)%dpth(3)*dHprime(n - 1))
            jac(2, n) = jac(2, n) + si%dt*ahat(1, l)* &
                        (fs(l)%d2H(9) - fs(l)%d2pth(9)*Hprime(l) - &
                         fs(l)%dpth(3)*dHprime(n))
        end do
        ...
        do k = 2, s
            ...
            jac(m - 3, 1) = jac(m - 3, 1) + si%dt*ahat(k, l)* &              ! d/dr
                            (fs(l)%d2H(2) - fs(l)%d2pth(2)*Hprime(l) - &
                             fs(l)%dpth(2)*dHprime(1))
            jac(m - 3, 2) = jac(m - 3, 2) + si%dt*ahat(k, l)* &
                            ! d/dpph
                            (fs(l)%d2H(8) - fs(l)%d2pth(8)*Hprime(l) - &
                             fs(l)%dpth(2)*dHprime(2))
            ...
            jac(m - 1, 1) = jac(m - 1, 1) - si%dt*a(k, l)* & ! d/dr
                            (-fs(l)%dhph(1)*(fs(l)%vpar - &
                                             Hprime(l)*fs(l)%hth)/fs(l)%hph**2 &
                             + (fs(l)%dvpar(1) - dHprime(1)*fs(l)%hth - &
                                Hprime(l)*fs(l)%dhth(1))/fs(l)%hph)
            ...
            jac(m, 1) = jac(m, 1) + si%dt*ahat(k, l)* & ! d/dr
                        (fs(l)%d2H(3) - fs(l)%d2pth(3)*Hprime(l) - &
                         fs(l)%dpth(3)*dHprime(1))
            jac(m, 2) = jac(m, 2) + si%dt*ahat(k, l)* & ! d/dpph
                        (fs(l)%d2H(9) - fs(l)%d2pth(9)*Hprime(l) - &
                         fs(l)%dpth(3)*dHprime(2))
        end do
        ...
        do l = 2, s
            do k = 2, s
                m = 4*k - 2
                n = 4*l - 2
                jac(m - 3, n - 3) = jac(m - 3, n - 3) + si%dt*ahat(k, l)* &
                                    ! d/dr
                                    (fs(l)%d2H(2) - fs(l)%d2pth(2)*Hprime(l) - &
                                     fs(l)%dpth(2)*dHprime(n - 3))
                jac(m - 3, n - 2) = jac(m - 3, n - 2) + si%dt*ahat(k, l)* &
                                    ! d/dth
                                    (fs(l)%d2H(4) - fs(l)%d2pth(4)*Hprime(l) - &
                                     fs(l)%dpth(2)*dHprime(n - 2))
                jac(m - 3, n - 1) = jac(m - 3, n - 1) + si%dt*ahat(k, l)* &
                                    ! d/dph
                                    (fs(l)%d2H(5) - fs(l)%d2pth(5)*Hprime(l) - &
                                     fs(l)%dpth(2)*dHprime(n - 1))
                jac(m - 3, n) = jac(m - 3, n) + si%dt*ahat(k, l)* &
                                ! d/dpph
                                (fs(l)%d2H(8) - fs(l)%d2pth(8)*Hprime(l) - &
                                 fs(l)%dpth(2)*dHprime(n))
                ...
                ! d/dpph
                jac(m - 1, n) = jac(m - 1, n) - si%dt*a(k, l)* &
    ...
    subroutine orbit_timestep_sympl_expl_impl_euler(si, f, ierr)
        ...
        do while (ktau .lt. si%ntau)
            ...
            if (extrap_field) then
                f%pth = f%pth + f%dpth(1)*(x(1) - xlast(1)) + &
                        f%dpth(4)*(x(2) - xlast(2))
                f%dH(1) = f%dH(1) + f%d2H(1)*(x(1) - xlast(1)) + &
                          f%d2H(7)*(x(2) - xlast(2))
                f%dpth(1) = f%dpth(1) + f%d2pth(1)*(x(1) - xlast(1)) + &
                            f%d2pth(7)*(x(2) - xlast(2))
                f%vpar = f%vpar + f%dvpar(1)*(x(1) - xlast(1)) + &
                         f%dvpar(4)*(x(2) - xlast(2))
    ...
    subroutine orbit_timestep_sympl_impl_expl_euler(si, f, ierr)
        ...
        do while (ktau .lt. si%ntau)
            ...
            if (extrap_field) then
                ...
                f%dpth(1) = f%dpth(1) + f%d2pth(1)*dz(1) + f%d2pth(2)*dz(2) + &
                            f%d2pth(3)*dz(3)
                f%dpth(2) = f%dpth(2) + f%d2pth(2)*dz(1) + f%d2pth(4)*dz(2) + &
                            f%d2pth(5)*dz(3)
                f%dpth(3) = f%dpth(3) + f%d2pth(3)*dz(1) + f%d2pth(5)*dz(2) + &
                            f%d2pth(6)*dz(3)
            else
                call eval_field(f, si%z(1), si%z(2), si%z(3), 0)
                call get_derivatives(f, si%z(4))
    ...
    subroutine orbit_timestep_sympl_rk_gauss(si, f, s, ierr)
        ...
        do while (ktau .lt. si%ntau)
            ...
            do l = 1, s
                ...
                si%z(3) = si%z(3) + si%dt*b(l)*(fs(l)%vpar - &
    ...
    subroutine orbit_timestep_sympl_rk_lobatto(si, f, s, ierr)
        ...
        do while (ktau .lt. si%ntau)
            ...
            do k = 2, s
                call eval_field(fs(k), x(4*k - 3 - 2), x(4*k - 2 - 2), x(4*k &
                                                                         - 1 - 2), 0)
