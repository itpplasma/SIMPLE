...
!> mode_secders = 2: all second derivatives, including mixed
   subroutine eval_field_can(f, r, th_c, ph_c, mode_secders)
      ...
      call splint_can_coord(.false., mode_secders, r, th_c, ph_c, &
                            ...
                            d2sqg(1), d2sqg(2), d2sqg(3), d2sqg(4), d2sqg(5), &
                            d2sqg(6), &
                            d2Bth(1), d2Bth(2), d2Bth(3), d2Bth(4), d2Bth(5), &
                            d2Bth(6), &
                            d2Bph(1), d2Bph(2), d2Bph(3), d2Bph(4), d2Bph(5), &
                            d2Bph(6), dummy)
      ...
      if (mode_secders == 2) then
         ...
         f%d2hth((/4, 6/)) = d2Bth((/4, 6/))/f%Bmod &
                             ...
                             + Bth/bmod2*(2.0_dp*f%dBmod((/2, 3/))**2/f%Bmod - &
                                          f%d2Bmod((/4, 6/)))
         f%d2hph((/4, 6/)) = d2Bph((/4, 6/))/f%Bmod &
                             - 2.0_dp*dBph((/2, 3/))*f%dBmod((/2, 3/))/bmod2 &
                             + Bph/bmod2*(2.0_dp*f%dBmod((/2, 3/))**2/f%Bmod - &
                                          f%d2Bmod((/4, 6/)))
      end if
      ...
      if (mode_secders == 2 .or. mode_secders == 3) then
         ...
         if (mode_secders == 2) then
            ...
            f%d2hth(5) = d2Bth(5)/f%Bmod &
                         ...
                         + Bth/bmod2*(2.0_dp*f%dBmod(2)*f%dBmod(3)/f%Bmod - &
                                      f%d2Bmod(5))
            f%d2hph(5) = d2Bph(5)/f%Bmod &
                         - (dBph(2)*f%dBmod(3) + dBph(3)*f%dBmod(2))/bmod2 &
                         + Bph/bmod2*(2.0_dp*f%dBmod(2)*f%dBmod(3)/f%Bmod - &
                                      f%d2Bmod(5))
         end if
      end if
