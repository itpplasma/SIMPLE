...
contains
   ...
   subroutine build_splines(source, ref_coords, spl, scaling, &
      ...
      h_r = (xmax(1) - xmin(1))/real(max(1, n_r - 1), dp)
      h_th = (xmax(2) - xmin(2))/real(max(1, n_th), dp)
      h_phi = (xmax(3) - xmin(3))/real(max(1, n_phi), dp)
   ...
   subroutine configure_spline_grid(ref_coords, n_r_in, n_th_in, n_phi_in, xmin_in, &
      ...
      type is (gvec_coordinate_system_t)
         if (.not. present(n_r_in)) n_r = cs%data%geom_splines(1)%num_points(1)
         if (.not. present(n_th_in)) n_th = cs%data%geom_splines(1)%num_points(2)
         if (.not. present(n_phi_in)) n_phi = cs%data%geom_splines(1)%num_points(3)
      class default
         continue
      end select
      ...
      type is (gvec_coordinate_system_t)
         block
            ...
            s_min = cs%data%geom_splines(1)%x_min(1)
            s_max = s_min + cs%data%geom_splines(1)%h_step(1)* &
                    real(cs%data%geom_splines(1)%num_points(1) - 1, dp)

            if (.not. present(xmin_in)) xmin(1) = max(xmin(1), sqrt(max(0.0_dp, s_min)))
            if (.not. present(xmax_in)) xmax(1) = min(xmax(1), sqrt(max(0.0_dp, s_max)))
         end block
      class default
         continue
      end select
...
!$omp end parallel
   ...
   subroutine evaluate_at_ref_coords(source, ref_coords, x_scaled, x_ref, x_cart, &
      ...
      type is (gvec_field_t)
         select type (ref_coords)
         type is (gvec_coordinate_system_t)
            if (source%data%same_family(ref_coords%data)) then
               call source%evaluate(x_ref, Acov, hcov, Bmod)
               x_cart = 0d0
               e_cov = 0d0
               needs_scaling = .true.
