...
contains
  ...
  subroutine vmec_field_evaluate_with_field(field, s, theta, varphi, &
    ...
    type is (GvecField) 
      ! For GVEC, we need to compute the required quantities
      ! Many of these are available through GVEC's internal structures
      r = sqrt(s)
      ds_dr = 2.0_dp * r
  ...
  subroutine vmec_iota_interpolate_with_field(field, s, aiota, daiota_ds)
    ...
    type is (GvecField)
      ! For GVEC, we need to access the iota profile
      ! This requires using GVEC's eval_iota_r function
      r = sqrt(s)
      aiota = eval_iota_r(r)
  ...
  subroutine vmec_lambda_interpolate_with_field(field, s, theta, varphi, alam, dl_dt)
    ...
    type is (GvecField)
      ! For GVEC, Lambda (stream function) is available as LA
      ! Need to evaluate it at the given coordinates
      ! Check if GVEC state is initialized
      if (.not. allocated(LA_r)) then
        ! GVEC state not loaded - return zeros
        ! Cannot reload here as field is INTENT(IN)
        alam = 0.0_dp
        dl_dt = 0.0_dp
        return
      end if
  ...
  subroutine vmec_data_interpolate_with_field(field, s, theta, varphi, &
    ...
    type is (GvecField)
      ! For GVEC, we can get R, Z and other quantities
      r_loc = sqrt(s)
