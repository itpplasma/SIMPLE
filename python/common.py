...
# Specify save location for plots
autosave = 0
folderforPlots = r'/mnt/c/Users/alexe/Desktop/Pictures Master'

taub = 7800 # estimated bounce time

dt_cp_sym = [0.01, 0.1, 1, 10, 100]
dt_cpp_sym = [0.01, 10, 100, 500, 1000]
dt_cpp_var = [0.01, 10, 100, 500, 1000]
dt_gc_var = [0.01, 10, 100, 500, 1000]
timesteps = 15000

#tint = [0.01, 0.05, 0.1, 1, 10, 100, 1000, 1200]
#dt_cp_sym = tint
#dt_cpp_sym = tint
#dt_cpp_var = tint
#dt_gc_var = tint
...
pph0 = m*vpar0*f.co_hph + qe/c*f.co_Aph
...
def get_val(z):
    ...
    vpar = 1.0/f.co_hph*(z[3]-qe/c*f.co_Aph)
    H = m*vpar**2/2 + mu*f.B + qe*f.Phie
    pth = m*f.co_hth*vpar + qe/c*f.co_Ath
    ret = [H, pth, vpar]
    return ret
