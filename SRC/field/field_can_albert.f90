...
use field_can_meiss, only: xmin, xmax, n_r, n_th, n_phi, order, periodic, twopi, &
    get_grid_point, generate_regular_grid, spl_Ath, spl_Aph, init
...
subroutine init_splines_with_psi
    ...
    call grid_r_to_psi(n_r, n_th, n_phi, xmin(1), xmax(1), psi_inner, psi_outer, &
        psi_of_x, Acan(3,:,:,:), hcan(3,:,:,:), hcan(2,:,:,:), Bmod, &
