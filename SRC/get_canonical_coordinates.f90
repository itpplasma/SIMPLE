...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
  subroutine splint_can_coord_moreders(r,vartheta_c,varphi_c,                                           &
                                       A_theta,A_phi,dA_theta_dr,dA_phi_dr,d2A_phi_dr2,                 &
                                       sqg_c,dsqg_c_dr,dsqg_c_dt,dsqg_c_dp,                             &
                                       B_vartheta_c,dB_vartheta_c_dr,dB_vartheta_c_dt,dB_vartheta_c_dp, &
                                       B_varphi_c,dB_varphi_c_dr,dB_varphi_c_dt,dB_varphi_c_dp,         &
...
!
  ...
  double precision, dimension(ns_max,ns_max) :: dstp_sqg_ds,dstp_bt_ds,dstp_bp_ds
!
  double precision, dimension(ns_max)        :: d2sp_sqg_ds2,d2sp_bt_ds2,d2sp_bp_ds2
  double precision, dimension(ns_max,ns_max) :: d2stp_sqg_ds2,d2stp_bt_ds2,d2stp_bp_ds2
!
  A_theta=torflux*r
  dA_theta_dr=torflux
!
...
!
  do k=ns_A,2,-1
    d2A_phi_dr2=sA_phi(k+1,is)*dfloat(k)*dfloat(k-1)+ds*d2A_phi_dr2
    if(k.gt.2) d3A_phi_dr3=sA_phi(k+1,is)*dfloat(k*(k-1)*(k-2))+ds*d3A_phi_dr3
  enddo
!
  rho_tor=sqrt(r)
  ds=rho_tor/hs_c
...
!
  ...
  dstp_bp_ds(1:nstp,1:nstp)=0.d0
!
  d2stp_sqg_ds2(1:nstp,1:nstp)=0.d0
  d2stp_bt_ds2(1:nstp,1:nstp)=0.d0
  d2stp_bp_ds2(1:nstp,1:nstp)=0.d0
!
  do k=ns_s_c,1,-1
    stp_sqg(1:nstp,1:nstp)=s_sqg_c(k,:,:,is,i_theta,i_phi)+ds*stp_sqg(1:nstp,1:nstp)
    dstp_sqg_ds(1:nstp,1:nstp)=s_sqg_c(k+1,:,:,is,i_theta,i_phi)*dfloat(k)+ds*dstp_sqg_ds(1:nstp,1:nstp)
...
!
    if(k.gt.1) then
      d2stp_sqg_ds2(1:nstp,1:nstp)=s_sqg_c(k+1,:,:,is,i_theta,i_phi)*dfloat(k*(k-1))+ds*d2stp_sqg_ds2(1:nstp,1:nstp)
      d2stp_bt_ds2(1:nstp,1:nstp)=s_B_vartheta_c(k+1,:,:,is,i_theta,i_phi)*dfloat(k*(k-1))+ds*d2stp_bt_ds2(1:nstp,1:nstp)
      d2stp_bp_ds2(1:nstp,1:nstp)=s_B_varphi_c(k+1,:,:,is,i_theta,i_phi)*dfloat(k*(k-1))+ds*d2stp_bp_ds2(1:nstp,1:nstp)
    endif
  enddo
!
! End interpolation over $s$
...
!
  ...
  dsp_bp_dt(1:nstp)=0.d0
!
  d2sp_sqg_ds2(1:nstp)=d2stp_sqg_ds2(nstp,1:nstp)
  d2sp_bt_ds2(1:nstp)=d2stp_bt_ds2(nstp,1:nstp)
  d2sp_bp_ds2(1:nstp)=d2stp_bp_ds2(nstp,1:nstp)
!
  do k=ns_tp_c,1,-1
    sp_sqg(1:nstp)=stp_sqg(k,1:nstp)+dtheta*sp_sqg(1:nstp)
    dsp_sqg_ds(1:nstp)=dstp_sqg_ds(k,1:nstp)+dtheta*dsp_sqg_ds(1:nstp)
...
!
    ...
    dsp_bp_dt(1:nstp)=stp_bp(k+1,1:nstp)*dfloat(k)+dtheta*dsp_bp_dt(1:nstp)
!
    d2sp_sqg_ds2(1:nstp)=d2stp_sqg_ds2(k,1:nstp)+dtheta*d2sp_sqg_ds2(1:nstp)
    d2sp_bt_ds2(1:nstp)=d2stp_bt_ds2(k,1:nstp)+dtheta*d2sp_bt_ds2(1:nstp)
    d2sp_bp_ds2(1:nstp)=d2stp_bp_ds2(k,1:nstp)+dtheta*d2sp_bp_ds2(1:nstp)
  enddo
!
! End interpolation over $\theta$
!--------------------------------
...
!
  ...
  dB_varphi_c_dp=0.d0
!
  d2sqg_c_dr2=d2sp_sqg_ds2(nstp)
  d2B_vartheta_c_dr2=d2sp_bt_ds2(nstp)
  d2B_varphi_c_dr2=d2sp_bp_ds2(nstp)
!
  do k=ns_tp_c,1,-1
    sqg_c=sp_sqg(k)+dphi*sqg_c
    dsqg_c_dr=dsp_sqg_ds(k)+dphi*dsqg_c_dr
...
!
    ...
    dB_varphi_c_dp=sp_bp(k+1)*dfloat(k)+dphi*dB_varphi_c_dp
!
    d2sqg_c_dr2=d2sp_sqg_ds2(k)+dphi*d2sqg_c_dr2
    d2B_vartheta_c_dr2=d2sp_bt_ds2(k)+dphi*d2B_vartheta_c_dr2
    d2B_varphi_c_dr2=d2sp_bp_ds2(k)+dphi*d2B_varphi_c_dr2
  enddo
!
! End interpolation over $\varphi$
!
  d2sqg_c_dr2=(d2sqg_c_dr2-dsqg_c_dr/rho_tor)/(4.d0*r)
  d2B_vartheta_c_dr2=(d2B_vartheta_c_dr2-dB_vartheta_c_dr/rho_tor)/(4.d0*r)
  d2B_varphi_c_dr2=(d2B_varphi_c_dr2-dB_varphi_c_dr/rho_tor)/(4.d0*r)
!
  dsqg_c_dr=0.5*dsqg_c_dr/rho_tor
  dB_vartheta_c_dr=0.5d0*dB_vartheta_c_dr/rho_tor
  dB_varphi_c_dr=0.5d0*dB_varphi_c_dr/rho_tor
!
  end subroutine splint_can_coord_moreders
!
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
