...
contains
  ...
  subroutine multi_particle_retention(nfail)
    ...
    ! Bug signature: nconf collapses and ntrap_conf = 0. Require a clear majority
    ! confined over this short trace, and at least one trapped retained.
    call check('multi-particle: majority confined over short trace', nconf >= 5, nfail)
    call check('multi-particle: at least one trapped retained', ntrap_conf >= 1, nfail)
