program test_cpp_canonical
  ...
  ! reference oracle (DVI_python, CORRECT metric). The oracle was regenerated by
  ! /tmp/dvi_oracle/oracle.py (numpy 2.4.6 / scipy 1.17.1, scipy.optimize.root
  ! hybr, tol=1e-12) after patching the python metric() d_33 theta-derivative
  ! (missing factor r). The Fortran uses the same CORRECT metric and must
  ! reproduce z(t) to ~1e-10 plus the symplectic energy bound:
  !   CP   dt=1   : per-step z to 1e-10
  !   CPP-sym dt=80: per-step z to 1e-10; max|dE/E0| ~1e-3, no secular drift
  !   CPP-var dt=800, ph0=1.0: per-step z to 1e-9
  ! The plateau across dt=80,40,20,10 (~1e-3, dt-independent) confirms the scheme
  ! is symplectic (bounded energy, no drift) -- the corrected metric, not the
  ! python listing's, is what produces it (python d_33 gives ~1.4e-1 with drift).
  ...
  call test_cpp_sym_plateau(nfail)
...
contains
  ...
  subroutine test_cpp_sym(nfail)
    ...
    call cpp_canon_init(st, MODEL_CPP_SYM, COORD_TOK, x0, 0.0_dp, 0.0_dp, mu, &
                        mass, charge, 80.0_dp)
    call step_check(st, 1, [9.920905973243960e-02_dp, 1.496995911614404e+00_dp, &
         -2.988543793214425e-03_dp], 'CPPsym step1', nfail, ierr)
    call step_check(st, 1, [9.841222045931407e-02_dp, 1.488567996867081e+00_dp, &
         -1.193466536209382e-02_dp], 'CPPsym step2', nfail, ierr)
    call step_check(st, 3, [9.602032387227967e-02_dp, 1.427433502330788e+00_dp, &
         -7.381976030782524e-02_dp], 'CPPsym step5', nfail, ierr)
    call step_check(st, 5, [9.197473170860219e-02_dp, 1.208988636963697e+00_dp, &
         -2.875834178193739e-01_dp], 'CPPsym step10', nfail, ierr)
    ...
    print '(A,ES12.4,A,ES12.4)', '  CPPsym max|dE/E0| = ', (Emax - Emin)/abs(E0), &
         '  end-drift = ', drift
    ! Oracle: max ~9.97e-4, end-drift ~-1.28e-5. No secular growth.
    call check('CPPsym energy bound ~1e-3', (Emax - Emin)/abs(E0) < 2.0e-3_dp, nfail)
    call check('CPPsym end-drift tiny (<1e-3)', abs(drift) < 1.0e-3_dp, nfail)
    call check('CPPsym not python-buggy (<5e-2, would be 1.4e-1)', &
               (Emax - Emin)/abs(E0) < 5.0e-2_dp, nfail)
  ...
  subroutine test_cpp_var(nfail)
    ...
    call step_check_tol(st, 1, [8.429882530519651e-02_dp, 9.298855308014161e-01_dp, &
         4.545081931389107e-01_dp], 'CPPvar step1', nfail, ierr, 1.0e-9_dp)
    call step_check_tol(st, 1, [8.810489162073597e-02_dp, -6.536514336499465e-02_dp, &
         -3.985853872228265e-01_dp], 'CPPvar step2', nfail, ierr, 1.0e-9_dp)
    ...
    call assert_vec(st%z(1:3), [9.556253758137316e-02_dp, -1.490693542329038e+00_dp, &
         8.136871364799693e+01_dp], 'CPPvar step2000', nfail, 1.0e-7_dp)
  end subroutine test_cpp_var

  subroutine test_cpp_sym_plateau(nfail)
    ! A true symplectic integrator has a bounded, roughly dt-independent energy
    ! plateau. The python listing's metric instead gives ~1.4e-1 with secular
    ! drift; the corrected metric gives ~1e-3 flat. Assert the plateau.
    integer, intent(inout) :: nfail
    real(dp) :: dts(4), plat(4)
    integer :: i, n, it, ierr
    type(cpp_canon_state_t) :: st
    ...
    do i = 1, 4
      ...
      plat(i) = (Emax - Emin)/abs(E0)
      print '(A,F6.1,A,ES12.4)', '  CPPsym plateau dt=', dts(i), '  max|dE/E0|=', plat(i)
    end do
    ! All four plateaus within a factor 2 of each other and all ~1e-3.
    call check('CPPsym plateau dt-independent (no secular growth)', &
               maxval(plat) < 2.0_dp*minval(plat) .and. maxval(plat) < 2.0e-3_dp, nfail)
  end subroutine test_cpp_sym_plateau
