program test_refcoords_file_detection
   ...
   call test_init_reference_coords_gvec(nerrors, 'wout.gvec_export.nc')
   call test_init_reference_coords_gvec(nerrors, 'wout.gvec_boozer_export.nc')
...
contains
   ...
   subroutine test_init_reference_coords_gvec(nerrors, filename)
      integer, intent(inout) :: nerrors
      ...
      print *, 'Test GVEC refcoords init for ', trim(filename)

      inquire (file=filename, exist=file_exists)
      if (.not. file_exists) then
         print *, '  FAIL: ', trim(filename), ' not found in test directory'
         nerrors = nerrors + 1
         return
      end if

      coord_input = filename
      ...
      if (x(1) <= 0.0_dp) then
         print *, '  FAILED: GVEC evaluate_cyl returned invalid R=', x(1)
         nerrors = nerrors + 1
         return
      end if

      print *, '  PASSED: ref_coords allocated and functional for ', trim(filename)
   end subroutine test_init_reference_coords_gvec
