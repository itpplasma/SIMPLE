program test_tokamak_meiss_transform
  ...
  if (isw_field_type /= 3) then
    error stop 'Tokamak config must request Meiss canonical field'
  end if

  call field_from_file('meiss', field_obj, .true.)

  select type(field_obj)
  type is (GeofluxField)
    continue
  class default
    error stop 'field_from_file did not return GeofluxField for Meiss setup'
  end select
  ...
  do i_phi_idx = 1, num_slices
    ...
    write(filename_png, '(A,I2.2,A)') 'tokamak_meiss_integrand_phi', i_phi, '.png'
    write(filename_py, '(A,I2.2,A)') 'tokamak_meiss_integrand_phi', i_phi, '.py'
    write(plot_title, '(A,I0,A)') 'Meiss integrand slice at phi index ', &
      i_phi, ' (tokamak)'
...
end program test_tokamak_meiss_transform
