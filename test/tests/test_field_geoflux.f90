program test_field_geoflux
    ...
    use field_geoflux, only: geoflux_field_t
    ...
    type is (geoflux_field_t)
        ...
        if (Bmod <= 0.0_dp) then
            error stop 'geoflux_field_t: Bmod must be positive'
        end if

        if (abs(Acov(3)) <= 0.0_dp) then
            error stop 'geoflux_field_t: Aphi expected to be non-zero away from axis'
        end if

        norm_sq = sum(hcov**2)
        if (norm_sq <= 0.0_dp) then
            error stop 'geoflux_field_t: hcov has zero magnitude'
        end if
    class default
        error stop 'field_from_file did not return geoflux_field_t for GEQDSK input'
    end select
