...
contains
    ...
    subroutine test_A_evaluate_roundtrip(nerrors)
        ...
        integer :: ierr, i, j
        integer, parameter :: n_r = 8, n_th = 16
        real(dp) :: r, theta, phi
        ...
        allocate(err_vmec_grid(n_th, n_r), err_chart_grid(n_th, n_r))

        do i = 1, n_r + 1
            r_grid(i) = 0.2_dp + 0.5_dp * real(i - 1, dp) / real(n_r, dp)
        end do
        do j = 1, n_th + 1
        ...
        print *, '  VMEC->chart mapping failures: ', n_vmec_fail, ' / ', n_r * n_th
        print *, '  Chartmap roundtrip failures: ', n_chart_fail, ' / ', n_r * n_th
    ...
    subroutine test_B_covariant_basis_fd(nerrors)
        ...
        real(dp), parameter :: tol_rel = 1.0e-3_dp
        logical :: vmec_exists, chartmap_exists
        integer :: i, j, k
        integer, parameter :: n_r = 8, n_th = 16
        real(dp) :: r, theta, phi
        ...
        do i = 1, n_r
            ...
            do j = 1, n_th
                ...
                do k = 1, 3
                    u_plus = u; u_plus(k) = u(k) + h
                    u_minus = u; u_minus(k) = u(k) - h
    ...
    subroutine test_C_basis_transform_identity(nerrors)
        ...
        do i = 1, n_r
            ...
            do j = 1, n_th
                ...
                if (err > tol) then
                    print *, '  Chartmap transform identity error at u=', u, ' err=', err
                    nerrors = nerrors + 1
                end if
            end do
        end do
