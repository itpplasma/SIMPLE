program test_soa_trace
    ...
    nerrors = 0
    do i = 1, npts
        if (reldiff(z_final(1, i), z_final_ref(1, i)) > RTOL) then
            print *, 'Mismatch z_r at i=', i, z_final(1, i), z_final_ref(1, i), &
                reldiff(z_final(1, i), z_final_ref(1, i))
            nerrors = nerrors + 1
        end if
        if (reldiff(z_final(2, i), z_final_ref(2, i)) > RTOL) then
            print *, 'Mismatch z_th at i=', i, z_final(2, i), z_final_ref(2, i), &
                reldiff(z_final(2, i), z_final_ref(2, i))
            nerrors = nerrors + 1
        end if
        if (reldiff(z_final(3, i), z_final_ref(3, i)) > RTOL) then
            print *, 'Mismatch z_ph at i=', i, z_final(3, i), z_final_ref(3, i), &
                reldiff(z_final(3, i), z_final_ref(3, i))
            nerrors = nerrors + 1
        end if
    end do
    ...
    print *, 'PASSED: trace_orbit_soa matches single-point version'
