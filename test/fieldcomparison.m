%% Initialize field
parentpath='/proj/plasma/CODE/NEO-ORB';
addpath([parentpath,'/matalb']);
...
addpath([parentpath,'/fastBSpline']);
...
javaaddpath([parentpath, '/netcdfAll-4.6.13.jar']);
data=read_vmec([parentpath, '/wout.nc']);


%% evaluation functions of VMEC quantities
J=@(s,u,v) abs(ceval(s,u,v,data.gmnc,data.xm,data.xn,0,0));
...
figure
plot(x,hhcompare(1,:),x,hhcompare(2,:),x,hhcompare(3,:));
xlabel('s/ fluxsurface label')
