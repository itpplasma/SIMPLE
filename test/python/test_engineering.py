...
class TestWallHeatMap:
    ...
    def test_flux_grid_shape(self, mock_results_file: Path):
        ...
        heat_map = WallHeatMap.from_netcdf(
            mock_results_file, n_theta=32, n_zeta=64
        )
    ...
    def test_total_power_positive(self, mock_results_file: Path):
        """Test total power is computed and positive."""
        heat_map = WallHeatMap.from_netcdf(mock_results_file)

        assert heat_map.total_power > 0
    ...
    def test_integrated_flux_in_region(self, mock_results_file: Path):
        ...
        assert full_power == pytest.approx(heat_map.total_power, rel=0.01)

        half_power = heat_map.integrated_flux_in_region(
            -np.pi, np.pi, 0, np.pi
        )
        assert half_power < heat_map.total_power
    ...
    def test_no_lost_particles(self, tmp_path: Path):
        ...
        heat_map = WallHeatMap.from_netcdf(results_path)
