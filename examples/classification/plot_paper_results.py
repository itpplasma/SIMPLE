#!/usr/bin/env python3
"""
Reproduce all classification figures from the JPP 2023 paper:
  Albert, Buchholz, Kasilov, Kernbichler, Rath, "Alpha particle confinement
  metrics based on orbit classification in stellarators",
  J. Plasma Phys. 89(3), 955890301 (2023). doi:10.1017/S0022377823000351
...
def plot_volume_classification(data_dir, output_path, config_label="",
    ...
    Bins trapped particles only (trap_par >= 0) into a (s, J_perp) grid.
    Passing particles are shown as white background. The trapped-passing
    and deeply-trapped boundaries from bminmax.dat are overlaid.

    classifier selects which iclass column drives the regular/prompt/
    chaotic colouring: "jpar" (column 3, J_parallel classifier) or
    "topology" (column 4, ideal/topological classifier). Both columns
    are populated in the same volume run, so the topology view is free.
    """
    if classifier == "jpar":
        class_col = 3
        title_prefix = "Volume classification (J_par)"
    elif classifier == "topology":
        class_col = 4
        title_prefix = "Volume classification (ideal topology)"
    else:
    ...
    icl = class_data[:, class_col].astype(int)
    ...
    ns, nperp = 50, 300
    hs = 1.0 / ns
    # Use the theoretical max of perp_inv = v_perp^2 / B_local for |p|=1
    # particles, which is 1 / bmin_global (achieved at the device-wide B
    # minimum). The sample maximum underestimates this when sampling is
    # finite, stretching the binned image and floating it above the
    # analytic boundary curves drawn from bmin_global / B_{min,max}(s).
    # See itpplasma/SIMPLE#349.
    pmax = 1.0 / np.min(bminmax[:, 1])
    hp = pmax / nperp
...
def main():
    ...
    # Volume classification for all 3 configs (J_par + topology, both
    # cheap because the iclass columns are written by the same run).
    volume_configs = [
        ("QH", "qh_volume"),
        ("QI", "qi_volume"),
        ("QA", "qa_volume"),
    ...
    for vol_label, vol_name in volume_configs:
        ...
        if not (vol_dir / "class_parts.dat").exists():
            continue
        for classifier, stem in [("jpar", "volume_classification"),
                                 ("topology", "volume_topology")]:
            out_name = f"{stem}_{vol_label.lower()}.pdf"
            print(f"\nGenerating {stem} plot ({vol_label})...")
            plot_volume_classification(
                vol_dir, output_dir / out_name, vol_label,
