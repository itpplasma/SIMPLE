...
# %%
tracy = ps.simple.Tracer()

ps.simple_main.init_field(tracy, "wout.nc", 3, 3, 3, 1)
ps.params.params_init()

# %% Initial conditions
z0_vmec = np.array([0.8, 1.0, 0.2, 1.0, 0.5])  # s, th, ph, v/v_th, v_par/v
z0_can = z0_vmec.copy()  # s, th_c, ph_c, v/v_th, v_par/v

z0_can[1:3] = ps.coord.vmec_to_can(z0_vmec[0], z0_vmec[1], z0_vmec[2])

ps.simple.init_sympl(
    tracy.si,
    tracy.f,
    z0_can,
    ps.params.dtaumin,
...
z_vmec[0, :] = z0_vmec
z_cyl[0, :2] = ps.coord.vmec_to_cyl(z_vmec[0, 0], z_vmec[0, 1], z_vmec[0, 2])
z_cyl[0, 2] = z_vmec[0, 2]

for kt in range(nt - 1):
    ps.orbit_symplectic.orbit_timestep_sympl_expl_impl_euler(tracy.si, tracy.f)
    z_integ[kt + 1, :] = tracy.si.z
    z_vmec[kt + 1, 0] = z_integ[kt + 1, 0]
    z_vmec[kt + 1, 1:3] = ps.coord.can_to_vmec(
    ...
    z_cyl[kt + 1, :2] = ps.coord.vmec_to_cyl(
