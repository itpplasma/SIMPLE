...
def get_test_data() -> str:
    cwd = os.getcwd()
    p = Popen(
        [cwd + "/../test/test_data/get_test_data.sh"],
        cwd=cwd + "/../test/test_data",
        stdout=PIPE,
        stdin=PIPE,
        stderr=PIPE,
        text=True,
...
def build_and_run_legacy():
    ...
    run(
        [
            "cp",
            "../test/test_data/wout.nc",
            "../test/test_data/simple_main/build/wout.nc",
        ]
    )
    ...
    run(
        [
            "cp",
            "../test/test_data/simple_main/build/times_lost.dat",
            "../test/tests/times_lost_old.dat",
        ]
    )
...
if __name__ == "__main__":
    ...
    cwd = os.getcwd()
    p = Popen(
        ["python", cwd + "/../test/tests/test_against_legacy_behaviour.py"],
        cwd=cwd + "/../test/tests/",
    )
    p.communicate()
    ## Clean artifacts
